import Vue from 'vue';
import {ADDSHOPITEM,CHANGESHOPITEMCOUNTMUTATION,CHANGESHOPITEMSELECTEDMUTATION,CHANGESELECTEDALLMUTATION} from '../mutation-type.js';
const state = {
	cartList:[
		{
			"selected":true,
...
const mutations = {
	[ADDSHOPITEM](state,good){
		//需要判断cartList中是否已经存在该商品  every some
	...
	[CHANGESHOPITEMCOUNTMUTATION](state,{num,index}){
		// console.log(num)
		// console.log('changeShopItemCountMutation')
		let shopItemCount=state.cartList[index].count;
		if(num>0){
...
const getters = {
	isSelectedAll(state){
		/*
		思路
			3.如果购物车中没有商品,全选按钮应该是未选中状态
			1.如果购物车中所有的商品都是选中状态,全选按钮应该是选中状态
			2.如果购物车中有任意一个商品是未选中状态,全选按钮应该是未选中状态
			
			4.返回值是布尔值
			5.为了知道购物车中所有商品的选中情况,我们需要进行遍历
